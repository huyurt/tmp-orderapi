...
using OrderApi.Identity;
using OrderApi.Schemas;
using OrderApi.Web.Chat.SignalR;
using OrderApi.Web.Common;
using OrderApi.Web.Resources;
using Swashbuckle.AspNetCore.Swagger;
using OrderApi.Web.IdentityServer;
using OrderApi.Web.Swagger;
using Stripe;
...
using OrderApi.Web.HealthCheck;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Microsoft.Extensions.DependencyInjection.Extensions;
using OrderApi.Web.Extensions;
using SecurityStampValidatorCallback = OrderApi.Identity.SecurityStampValidatorCallback;

namespace OrderApi.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<OrderApiWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (env.IsDevelopment())
            {
                ...
                app.UseOrderApiForwardedHeaders();
            }
            else
            {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "OrderApi API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("OrderApi.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "OrderApi API", Version = "v1"});
                ...
                var applicationXml = $"OrderApi.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"OrderApi.Web.Core.xml";
