...
namespace OrderApi.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : OrderApiServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("OrderApi", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
