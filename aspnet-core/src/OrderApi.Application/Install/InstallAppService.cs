...
using OrderApi.Migrations.Seed.Host;
...
namespace OrderApi.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : OrderApiAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<OrderApiDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{OrderApiConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{OrderApiConsts.ConnectionStringName}", constring);
        }
